...
{
    static APLocationManager *instance = nil;
    static dispatch_once_t onceToken;

    dispatch_once(&onceToken, ^{
        @synchronized(self) {
            instance = [[APLocationManager alloc] init];
        }
    });
...
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray<CLLocation *> *)locations
{
    ...
    [self.geocoder reverseGeocodeLocation:currentLocation completionHandler:^(NSArray<CLPlacemark *> * _Nullable placemarks, NSError * _Nullable error) {
        ...
        if (!error && placemark) {
            ...
            if ([weakSelf.delegate respondsToSelector:@selector(apLocationManager:didUpdateAPTripFinderLocation:)]) {
                APLocation *apLocation = [[APLocation alloc] init];
                ...
                [weakSelf.delegate apLocationManager:weakSelf didUpdateAPTripFinderLocation:apLocation];
            }
        }
    }];
}
