...
#import "APCacheManager.h"
#import "APCacheData.h"
...
@interface APCacheManager()
@property (strong, nonatomic) NSUserDefaults *userDefaults;
@property (strong, nonatomic) NSString *tutorialVersion;

@end

@implementation APCacheManager

+ (instancetype)sharedInstance
{
    static APCacheManager *instance = nil;
    static dispatch_once_t onceToken;

    dispatch_once(&onceToken, ^{
        @synchronized(self) {
            instance = [[APCacheManager alloc]init];
        }
    });
    return instance;
}
...
- (void)setDataWithKey:(NSString *)key dataType:(DATA_SAVE_FOR_TYPE)dataType data:(id)data cacheSecDuration:(NSInteger)cacheSecDuration
{
    ...
    APCacheData *cacheData = [[APCacheData alloc]initWithExprieSec:cacheSecDuration];
    cacheData.data = data;
    ...
    [APCacheManager setToKeyLists:key];
...
- (id)getDataWithKey:(NSString *)key
{
    ...
    @try {
        if (key && [self.userDefaults objectForKey:key]) {
            ...
            APCacheData *cacheData  =  (APCacheData *) [unarchiver decodeObjectOfClass:[APCacheData class] forKey:[[NSBundle mainBundle] bundleIdentifier]];
...
+ (void)setToKeyLists:(NSString *)key
{
    ...
    NSMutableArray *keys = [[NSMutableArray alloc]initWithArray:[[APCacheManager sharedInstance] getDataWithKey:keysName]];
